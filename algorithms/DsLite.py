...
def dstar_lite(
  ...
  def calculate_key(u) -> Tuple[float, float]:
    ...
    return (m + heuristic(source, u), m)
  
  def update_vertex(u, pq : PQ):
    # if u != source:
    if u != target:
      # rhs[u] = min((g_val(s) + w(s, u) for s in preds(u)), default = INF)
      rhs[u] = min((g_val(s) + w(u, s) for s in succs(u)), default = INF)
  ...
  pq.push(target, calculate_key(target))
  ...
  while True:
    ...
    if rhs_val(source) == g_val(source):
      # if pq_top_key() >= calculate_key(target):
      if pq_top_key() >= calculate_key(source):
        break
  ...
  if not math.isfinite(g_val(source)):
    return None
